...
df_model=df%>% select(incm, ho_incm, cfam, marri_1, D_1_1, D_2_1, LQ1_sb, LQ_1EQL, LQ_2EQL, LQ_3EQL, LQ_4EQL, LQ_5EQL, educ, EC1_1, BO1, BO1_1, BO2_1, BD1, BD1_11,  BP1,  BS1_1,  BE5_1, LK_EDU, LK_LB_CO, age,EC_wht_23,sex, HE_BMI, Ms, npins, tins)
...
df_model$Ms = factor(df_model$Ms)
df_model$npins=factor(df_model$npins)
df_model$tins = factor(df_model$tins)
...
set.seed(1000)
intrain = createDataPartition(y=df_model$Ms, p=0.7, list=FALSE)
...
library(rpart)
a=rpart.control(minsplit=20)
rpartmod=rpart(Ms~.,data=train, method="class")

rpartmod

plot(rpartmod)
text(rpartmod, pretty=0)
...
rpartpred_test = predict(rpartmod, test, type="class")
rpartpred_train = predict(rpartmod, train, type="class")


library("e1071")
confusionMatrix(rpartpred_test, test$Ms, positive="1")
confusionMatrix(rpartpred_train, train$Ms, positive="1")
